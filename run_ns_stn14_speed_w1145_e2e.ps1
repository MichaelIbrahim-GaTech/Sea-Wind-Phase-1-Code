...
Write-Host "[e2e] NS station d14 speed width-1.145 candidate"

$BaseCsv = Join-Path $Root "runs\v6_pressure_speed\pred_ecs14_w150.csv"
if ($RegenerateBase -or -not (Test-Path $BaseCsv)) {
    Write-Host "[e2e] generating ECS station d14 width-150 base first"
    $baseScript = Join-Path $Root "run_ecs_stn14_w150_e2e.ps1"
    $baseArgs = @()
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "ECS station d14 width base generation failed with exit code $LASTEXITCODE"
    }
}
...
& $Python (Join-Path $Root "build_ns_stn14_speed_w1145_candidate.py")
if ($LASTEXITCODE -ne 0) {
    throw "NS station d14 speed width build failed with exit code $LASTEXITCODE"
}

$OutZip = Join-Path $Root "runs\v6_pressure_speed\sub_ns14sw1145.zip"

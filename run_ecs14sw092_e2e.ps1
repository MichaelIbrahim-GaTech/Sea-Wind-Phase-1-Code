...
Write-Host "[e2e] ECS station d14 speed width-0.92 candidate"
...
& $Python (Join-Path $Root "build_ecs_stn14_speed_w092_candidate.py")
if ($LASTEXITCODE -ne 0) {
    throw "ECS station d14 speed width build failed with exit code $LASTEXITCODE"
}

$OutZip = Join-Path $Root "runs\v6_pressure_speed\sub_ecs14sw092.zip"

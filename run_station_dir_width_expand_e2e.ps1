...
Write-Host "[e2e] Station direction width expansion on speed-positive base"

$BaseCsv = Join-Path $Root "runs\v6_pressure_speed\pred_speedpos_wcal.csv"
if ($RegenerateBase -or -not (Test-Path $BaseCsv)) {
    Write-Host "[e2e] generating speed-positive width-calibration base first"
    $baseScript = Join-Path $Root "run_speedpos_width_cal_e2e.ps1"
    $baseArgs = @()
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Speed-positive base generation failed with exit code $LASTEXITCODE"
    }
}

if (-not (Test-Path $BaseCsv)) {
    throw "Expected speed-positive base CSV was not generated: $BaseCsv"
}

& $Python (Join-Path $Root "build_station_dir_width_expand_candidate.py")
if ($LASTEXITCODE -ne 0) {
    throw "Station direction width expansion build failed with exit code $LASTEXITCODE"
}

$OutZip = Join-Path $Root "runs\v6_pressure_speed\sub_stndir_wexp.zip"

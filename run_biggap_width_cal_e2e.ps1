...
Write-Host "[e2e] Big-gap width calibration candidate"
...
& $Python (Join-Path $Root "build_biggap_width_cal_candidate.py")
if ($LASTEXITCODE -ne 0) {
    throw "Big-gap width calibration build failed with exit code $LASTEXITCODE"
}

$OutZip = Join-Path $Root "runs\v6_pressure_speed\sub_biggap_wcal.zip"

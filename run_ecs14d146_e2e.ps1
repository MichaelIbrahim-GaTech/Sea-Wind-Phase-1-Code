...
Write-Host "[e2e] ECS station d14 exact direction-width 146 candidate"

$BaseCsv = Join-Path $Root "runs\v6_pressure_speed\pred_ecss14_w78.csv"
if ($RegenerateBase -or -not (Test-Path $BaseCsv)) {
    Write-Host "[e2e] generating current confirmed w78 base first"
    $baseScript = Join-Path $Root "run_ecss14w78_e2e.ps1"
    $baseArgs = @()
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Current confirmed w78 base generation failed with exit code $LASTEXITCODE"
    }
}

if (-not (Test-Path $BaseCsv)) {
    throw "Expected current confirmed base CSV was not generated: $BaseCsv"
}

& $Python (Join-Path $Root "build_ecs_stn14_d146_candidate.py")
if ($LASTEXITCODE -ne 0) {
    throw "ECS station d14 exact-width build failed with exit code $LASTEXITCODE"
}

$OutZip = Join-Path $Root "runs\v6_pressure_speed\sub_ecs14d146.zip"

...
Write-Host "[e2e] ECS surface d14 shrink-78 curve-fit candidate"

$BaseCsv = Join-Path $Root "runs\v6_pressure_speed\pred_ecss14_w80.csv"
if ($RegenerateBase -or -not (Test-Path $BaseCsv)) {
    Write-Host "[e2e] generating confirmed w80 base first"
    $baseScript = Join-Path $Root "run_ecss14w80_e2e.ps1"
    $baseArgs = @()
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Confirmed w80 base generation failed with exit code $LASTEXITCODE"
    }
}
...
& $Python (Join-Path $Root "build_ecs_surface_d14_w78_candidate.py")
if ($LASTEXITCODE -ne 0) {
    throw "ECS surface d14 shrink-78 build failed with exit code $LASTEXITCODE"
}

$OutZip = Join-Path $Root "runs\v6_pressure_speed\sub_ecss14w78.zip"

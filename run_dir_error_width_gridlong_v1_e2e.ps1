...
Write-Host "[e2e] direction error-width grid-long v1"

$required = @(
    "build_dir_error_width_gridlong_v1_candidate.py",
    ...
    "runs\v6_pressure_speed\pred_dir_error_width_newsignal_v1.csv",
...
python -m py_compile build_dir_error_width_gridlong_v1_candidate.py
if ($LASTEXITCODE -ne 0) {
    throw "Compile failed with exit code $LASTEXITCODE"
}

python build_dir_error_width_gridlong_v1_candidate.py
if ($LASTEXITCODE -ne 0) {
    throw "Builder failed with exit code $LASTEXITCODE"
}

$manifest = "runs\v6_pressure_speed\manifest_dir_error_width_gridlong_v1.json"

...
Write-Host "[e2e] ECS station d14 width-150 candidate"
...
& $Python (Join-Path $Root "build_ecs_stn14_w150_candidate.py")
if ($LASTEXITCODE -ne 0) {
    throw "ECS station d14 width build failed with exit code $LASTEXITCODE"
}

$OutZip = Join-Path $Root "runs\v6_pressure_speed\sub_ecs14_w150.zip"

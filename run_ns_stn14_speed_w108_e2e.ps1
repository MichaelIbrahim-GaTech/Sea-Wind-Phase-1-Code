...
Write-Host "[e2e] NS station d14 speed width-1.08 candidate"
...
& $Python (Join-Path $Root "build_ns_stn14_speed_w108_candidate.py")
if ($LASTEXITCODE -ne 0) {
    throw "NS station d14 speed width build failed with exit code $LASTEXITCODE"
}

$OutZip = Join-Path $Root "runs\v6_pressure_speed\sub_ns14sw108.zip"

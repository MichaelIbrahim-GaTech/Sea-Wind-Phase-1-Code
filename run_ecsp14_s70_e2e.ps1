...
Write-Host "[e2e] ECS pressure d14 shrink-70 candidate"

$BaseCsv = Join-Path $Root "runs\v6_pressure_speed\pred_ecs_stn14_w092.csv"
if ($RegenerateBase -or -not (Test-Path $BaseCsv)) {
    Write-Host "[e2e] generating confirmed pre-p14 base first"
    $baseScript = Join-Path $Root "run_ecs14sw092_e2e.ps1"
    $baseArgs = @()
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Confirmed pre-p14 base generation failed with exit code $LASTEXITCODE"
    }
}
...
& $Python (Join-Path $Root "build_ecs_pressure_d14_s70_candidate.py")
if ($LASTEXITCODE -ne 0) {
    throw "ECS pressure d14 shrink-70 build failed with exit code $LASTEXITCODE"
}

$OutZip = Join-Path $Root "runs\v6_pressure_speed\sub_ecsp14s70.zip"

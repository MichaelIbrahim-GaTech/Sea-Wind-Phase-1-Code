...
Write-Host "[e2e] NS surface d14 row-gate delta75 on promoted rg60 base"

$ConfirmedBaseCsv = Join-Path $Root "runs\v6_pressure_speed\pred_ns_sfc14_rg60_d146.csv"
if ($RegenerateBase -or -not (Test-Path $ConfirmedBaseCsv)) {
    Write-Host "[e2e] generating promoted rg60 base first"
    $baseScript = Join-Path $Root "run_ns_sfc14_rg60_d146_e2e.ps1"
    $baseArgs = @()
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Promoted rg60 base generation failed with exit code $LASTEXITCODE"
    }
}
...
& $Python (Join-Path $Root "build_ns_sfc14_rg75_candidate.py")
if ($LASTEXITCODE -ne 0) {
    throw "NS surface d14 rg75 candidate build failed with exit code $LASTEXITCODE"
}

$OutZip = Join-Path $Root "runs\v6_pressure_speed\sub_nssfc14rg75.zip"

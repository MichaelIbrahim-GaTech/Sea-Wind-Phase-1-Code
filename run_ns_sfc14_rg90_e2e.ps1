...
Write-Host "[e2e] NS surface d14 row-gate delta90 on promoted rg75 base"

$ConfirmedBaseCsv = Join-Path $Root "runs\v6_pressure_speed\pred_ns_sfc14_rg75.csv"
if ($RegenerateBase -or -not (Test-Path $ConfirmedBaseCsv)) {
    Write-Host "[e2e] generating promoted rg75 base first"
    $baseScript = Join-Path $Root "run_ns_sfc14_rg75_e2e.ps1"
    $baseArgs = @()
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Promoted rg75 base generation failed with exit code $LASTEXITCODE"
    }
}
...
& $Python (Join-Path $Root "build_ns_sfc14_rg90_candidate.py")
if ($LASTEXITCODE -ne 0) {
    throw "NS surface d14 rg90 candidate build failed with exit code $LASTEXITCODE"
}

$OutZip = Join-Path $Root "runs\v6_pressure_speed\sub_nssfc14rg90.zip"

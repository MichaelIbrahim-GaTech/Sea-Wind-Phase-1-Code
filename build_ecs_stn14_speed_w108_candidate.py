...
BASE_CSV = WORK / "pred_ns_stn14_w1145.csv"
OUT_CSV = WORK / "pred_ecs_stn14_w108.csv"
OUT_ZIP = WORK / "sub_ecs14sw108.zip"
MANIFEST = WORK / "manifest_ecs_stn14_w108.json"
...
def apply_width(after: pd.DataFrame) -> dict[str, object]:
    mask = after["type"].eq("station") & after["region"].eq(REGION) & after["horizon"].eq(HORIZON)
    idx = after.index[mask]
    if len(idx) != EXPECTED_ROWS:
        raise SystemExit(f"expected {EXPECTED_ROWS} target rows, got {len(idx)}")
    ...
    return {
        "name": f"ecs_station_d14_speed_width_{scale_tag()}",
        "rows": int(len(idx)),
        "region": REGION,
        "horizon": HORIZON,
...
def validate(before: pd.DataFrame, after: pd.DataFrame) -> dict[str, object]:
    ...
    if int(speed_changed.sum()) != EXPECTED_ROWS:
        raise SystemExit(f"expected exactly {EXPECTED_ROWS} speed interval rows changed; got {int(speed_changed.sum())}")
...
def main() -> None:
    if not BASE_CSV.exists():
        raise SystemExit(f"Missing base CSV: {BASE_CSV}. Run .\\run_ns_stn14_speed_w1145_e2e.ps1 first.")
    ...
    manifest = {
        ...
        "public_feedback_basis": {
            ...
            "target": "WS ECS Stations d14",
            "current_public_score": 9.0772,
            "leader_public_score": 8.84,
            "rejected_center_model": "run_ecs14spd_bag_e2e.ps1 failed its chronological CV gate",
            "rationale": "Width-only interval expansion keeps q50 unchanged and tests undercoverage without the failed center replacement.",
        },
        "code_hashes": {
            "build_ecs_stn14_speed_w108_candidate.py": sha256(Path(__file__).resolve()),
            "run_ecs14sw108_e2e.ps1": sha256(ROOT / "run_ecs14sw108_e2e.ps1"),
        },
        "competition_rule_notes": [
            "Uses only official-data-derived base predictions generated by the end-to-end pipeline.",
            "No target labels from the scoring server are used.",
            "No external data.",
            "Only ECS station horizon-14 speed intervals are changed; q50 and all direction predictions are preserved.",
        ],
    }

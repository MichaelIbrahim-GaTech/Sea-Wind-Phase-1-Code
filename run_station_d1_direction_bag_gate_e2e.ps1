...
Write-Host "[e2e] Station d1 direction bagging gate"
...
$BaseCsv = Join-Path $Root "runs\v6_pressure_speed\pred_ns_d1spd_calib_gate.csv"
if ($RegenerateBase -or -not (Test-Path $BaseCsv)) {
    Write-Host "[e2e] regenerating current accepted base first"
    $baseScript = Join-Path $Root "run_ns_station_d1_speed_calib_gate_e2e.ps1"
    $baseArgs = @()
...
Write-Host "[e2e] building gated station d1 direction bagged candidate"
& $Python (Join-Path $Root "build_station_d1_direction_bag_gate_candidate.py")
if ($LASTEXITCODE -ne 0) {
    throw "Candidate generation failed with exit code $LASTEXITCODE"
}

$ZipPath = Join-Path $Root "runs\v6_pressure_speed\sub_stndir_d1_bag_gate.zip"

...
Write-Host "[e2e] NS pressure d7 MOS-residual direction candidate"
...
$BaseCsv = Join-Path $Root "runs\v6_pressure_speed\pred_ns_p14dir_hres.csv"
if ($RegenerateBase -or -not (Test-Path $BaseCsv)) {
    Write-Host "[e2e] regenerating current best base first"
    $baseScript = Join-Path $Root "run_ns_p14dir_hres_e2e.ps1"
    $baseArgs = @()
...
Write-Host "[e2e] building NS pressure d7 MOS-residual direction candidate"
& $Python (Join-Path $Root "build_ns_p7dir_mosres_candidate.py")
if ($LASTEXITCODE -ne 0) {
    throw "Candidate generation failed with exit code $LASTEXITCODE"
}

$ZipPath = Join-Path $Root "runs\v6_pressure_speed\sub_ns_p7dir_mosres.zip"

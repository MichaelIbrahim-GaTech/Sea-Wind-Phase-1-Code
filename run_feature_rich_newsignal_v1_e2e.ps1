...
Write-Host "[e2e] feature-rich official-data new-signal v1"
...
missing = [m for m in ("lightgbm", "pyarrow", "pandas", "numpy") if importlib.util.find_spec(m) is None]
if missing:
...
python -m py_compile build_feature_rich_newsignal_v1_candidate.py
if ($LASTEXITCODE -ne 0) {
    throw "Compile failed with exit code $LASTEXITCODE"
}

python build_feature_rich_newsignal_v1_candidate.py
if ($LASTEXITCODE -ne 0) {
    throw "Builder failed with exit code $LASTEXITCODE"
}

$manifest = "runs\v6_pressure_speed\manifest_feature_rich_newsignal_v1.json"

...
Write-Host "[e2e] ECS station d14 speed bag gate candidate"
...
& $Python (Join-Path $Root "build_ecs_station_d14_speed_bag_gate_candidate.py")
if ($LASTEXITCODE -ne 0) {
    throw "ECS station d14 speed bag gate failed with exit code $LASTEXITCODE"
}

$OutZip = Join-Path $Root "runs\v6_pressure_speed\sub_ecs14spd_bag.zip"
